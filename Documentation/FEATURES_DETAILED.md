...
| **Logic & Algorithms** | Runs in `FixedUpdate`. Searches for colliders in a radius defined by the Shop's `GRadius` and `_repelRadiusOffset`. If the shop is not active, animating, or being dragged, it repels dynamic rigidbodies (balls) via `AddForce` and kinematic rigidbodies (machines) via `MovePosition`. |
